...
// Added input parameters controlling the algorithm
vector<Scheduled> solve_sa(const vector<Task>& tasks, int MaxProcs, double in_T_END, double in_alpha, double in_swap_prob) {
    ...
    // Using parameter passed from main
    const double T_END = in_T_END;      
    ...
    cerr << "[SA] Greedy start makespan = " << best_cost << "\n";
    cerr << "[SA] Params: T_END=" << T_END << ", Alpha=" << in_alpha << ", BaseSwapProb=" << in_swap_prob << "\n";

    // Adaptive temperature: scale by potential improvement range
    // double T = 0.30 * double(current_cost); 
    double T = 100000; 
    double T_start = T;
    
    // Using parameter passed from main
    double alpha = in_alpha;
   
    int no_improve_count = 0;
    long long last_improve_cost = best_cost;
    int iter = 0;

    // SA main loop with adaptive move selection
    while (T >= T_END) {
        iter++;
        double elapsed = chrono::duration<double>(chrono::high_resolution_clock::now() - time_start).count();
        if (elapsed > TIME_LIMIT) {
            cerr << "[SA] Time limit reached after " << elapsed << " s, iter=" << iter << "\n";
            break;
        }

        // Adaptive move strategy: increase swaps when stuck
        // Using input parameter as base
        double swap_prob = in_swap_prob;
        if (no_improve_count > 500) {
            // Increase swap probability when stuck, but don't exceed reasonable limits
            swap_prob = min(0.95, in_swap_prob + 0.10); 
        }
        ...
        if (accept) {
            ...
            if (neigh_cost < best_cost) {
                ...
                best_prior = neigh;
                no_improve_count = 0;
                cerr << "[SA] New best = " << best_cost << " at iter " << iter << " T=" << T << "\n";
            } else {
                no_improve_count++;
            }
        } else {
    ...
    cerr << "[SA] Final best makespan = " << best_cost << "\n";
...
int main(int argc, char* argv[]) {
    ...
    if(argc<3){
        cerr<<"Usage: "<<argv[0]<<" file.swf N [T_END] [ALPHA] [SWAP_PROB]\n";
        cerr<<"Defaults: T_END=1e-12, ALPHA=0.995, SWAP_PROB=0.75\n";
        return 1;
    }
    ...
    // Reading parameters from command-line arguments (if they exist)
    double arg_t_end = 1e-12;
    double arg_alpha = 0.995;
    double arg_swap_prob = 0.75;

    if (argc > 3) arg_t_end = stod(argv[3]);
    if (argc > 4) arg_alpha = stod(argv[4]);
    if (argc > 5) arg_swap_prob = stod(argv[5]);
    ...
    // Passing parameters to the solving function
    auto schedule = solve_sa(tasks_all, MaxProcs, arg_t_end, arg_alpha, arg_swap_prob);

    ofstream fout("output_meta.swf");
    if (!fout) {
        cerr << "Cannot create file output_meta.swf\n";
        return 1;
    }
    for(auto &s: schedule){
